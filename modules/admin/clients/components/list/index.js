...
export default {
  ...
  data() {
    return {
      titlePage: this.$t('admin.clients'),
      collection: [],
      isPaginated: true,
      searchValue: '',
      total: 0,
      ...
      customEvents: [
        ...
        { eventName: 'event-delete-clients', callback: this.handleDelete }
      ],
      form: {
        reason: "",
        id: ""
  ...
  computed: {
    titleStack() {
      return [this.$t('admin.advertisements')]
    },
    ...mapState({
    ...
    */
    ...
    handleToggleStatus(id, status) {
      if (!status) {
        ...
        this.form.id = id;
        this.$modal.show('reason-inactive');
      }
    },

    deactivateClient() {
  
        this.$ClientService.toggleStatus(this.form.id,this.form)
          .then((response) => {
            this.$modal.hide('reason-inactive');
            //* update list to remove this row *//
            this.collection.forEach((element, index) => {
              if (element.id === response.id) {
                this.collection[index] = Object.assign(this.collection[index], response)
